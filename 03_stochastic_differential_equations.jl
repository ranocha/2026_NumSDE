...
# ╔═╡ e905250c-cd96-4740-b971-b22880763529
md"""
...
First, we show the densities of the solution at different times.
"""

# ╔═╡ 36ae7ca8-6445-4f90-b9cf-8d81b901e29a
function geometric_brownian_motion_density(x, t, μ, σ)
	X0 = 1.0
	
	num = exp(-(log(x / X0) - (μ - 0.5 * σ^2) * t)^2 / (2 * t * σ^2))
	den = x * σ * sqrt(2 * π * t)
	return num / den
end

# ╔═╡ f5179884-b276-4e1f-b0bb-dcda82f46ccb
function plot_geometric_brownian_motion_density(; t, μ, σs)
	x = range(0.0, 4.0, length = 1000)
	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"x", ylabel = L"Density $p(x)$", title = L"t = %$t")

	for σ in σs
		lines!(ax, x, geometric_brownian_motion_density.(x, t, μ, σ);
			   label = L"\sigma = %$(σ)", linewidth = 2)
	end
...
# ╔═╡ b8f5d375-a830-487d-b39e-55599d3cb046
plot_geometric_brownian_motion_density(t = 1.0, μ = 0.05, σs = (0.3, 0.5))

# ╔═╡ 5f538b82-93bc-4b96-8c4d-ac43a50bf15e
plot_geometric_brownian_motion_density(t = 5.0, μ = 0.05, σs = (0.3, 0.5))

# ╔═╡ e1eafa55-0e9f-4b2b-83ef-1539de965a74
md"""
Next, we plot some realizations of solutions for fixed Brownian paths and
different values of the volatility $\sigma$.
"""

# ╔═╡ 18eb0124-5f37-4498-96ca-4af428e07f94
function plot_geometric_brownian_motion(; μ, σ)
	T = 1.0
	L = 500
	X0 = 1.0
	...
	for _ in 1:10
		δW = sqrt(δt) * randn(L + 1)
		δW[begin] = 0
		W = cumsum(δW)

		X = @. X0 * exp((μ - 0.5 * σ^2) * t + σ * W)
		lines!(ax, t, X; linestyle = :dash)
	end

	# Plot expected value
	X = @. X0 * exp(μ * t)
	lines!(ax, t, X; label = "expected value", linewidth = 5)

	axislegend(ax; position = :lt)
	return fig
end

# ╔═╡ e1dbb7b2-92bb-43f7-933b-df4d2a818187
let
	Random.seed!(42)
	plot_geometric_brownian_motion(μ = 1.0, σ = 0.2)
end
