...
# ╔═╡ 8f7b7a09-9508-4d66-b3af-690073a7a59f
begin
	using CairoMakie
...
# ╔═╡ 5961b5ec-c193-40ab-82f4-f1dab61f1452
using Statistics

# ╔═╡ 76beb307-24a4-4b21-a7be-38d960efd915
using StochasticDiffEq

# ╔═╡ 2678e678-9b76-4cd8-8721-5a746c6a08d9
using SpecialFunctions: erfc

# ╔═╡ ce2411d2-81fb-11ee-0441-7d4bb926681f
md"""
# 10 Multilevel Monte Carlo methods
...
We want to approximate $\mathbb{E}\bigl[ X(T) \bigr]$ where $X(t)$ is the solution of the SDE

$$\begin{equation*}
  \mathrm{d} X(t)
  =
  \mu X(t) \mathrm{d} t
  + \sigma X(t) \mathrm{d} W(t),
\end{equation*}$$

with parameters

- interest rate $\mu = 0.05$
- volatility $\sigma = 0.25$
- initial value $X(0) = 100$
- expiry time $T = 1$
"""

# ╔═╡ 1a5e516c-d597-4462-8bb1-722a77c3b3ec
md"""
First, we compute a single numerical approximation.
"""

# ╔═╡ 96d03839-75a4-4aec-b82d-b0b243901c03
f(X, p, t) = p.μ * X

# ╔═╡ 8f59c907-0721-46d8-be75-3f5766138840
g(X, p, t) = p.σ * X

# ╔═╡ aaa19826-ce3c-4d98-8bdd-deff997469bd
h(X, p, t) = X

# ╔═╡ e283cd7c-9fb8-4458-a59f-8c5e9b9ad8c4
let
	Random.seed!(42)
	parameters = (μ = 0.05, σ = 0.25)

	X0 = 100.0
	tspan = (0.0, 1.0)
	sde = SDEProblem(f, g, X0, tspan, parameters)
	sol = solve(sde, RKMilCommute())
	fig = Figure()
...
# ╔═╡ dd24511d-a2ee-4d7c-a49a-7ff77754584c
md"""
Next, we perform a standard Monte Carlo simulation using StochasticDifferentialEquations.jl.
"""

# ╔═╡ c56ec2b1-8852-4f78-b861-a746c0740cc4
function exact_value(X0, parameters, tspan)
	T = last(tspan)
	(; μ) = parameters
	return X0 * exp(μ * T)
end

# ╔═╡ 05439848-5e6c-41c1-9f4e-e5a444ac3dc4
md"""
First, we use a Runge-Kutta-like variant of the Milstein method with
adaptive step sizes.
"""

# ╔═╡ 83f60f68-e76e-4c7a-b1ad-3d3d79311f5e
let
	Random.seed!(42)
	...
	tspan = (0.0, 1.0)
	sde = SDEProblem(f, g, X0, tspan, parameters)
	ensemble = EnsembleProblem(sde;
		output_func = (sol, i) -> (h(sol.u[end], parameters, last(tspan)), false))
	sol = solve(ensemble, RKMilCommute(), EnsembleThreads();
	            save_everystep = false,
				trajectories = 10^4)

	md"""
	- Sample mean: $(round(mean(sol.u), digits = 3))
	- Std. dev.: $(round(std(sol.u), digits = 2))
	- Exact value: $(round(exact_value(X0, parameters, tspan), digits = 3))
	"""
end

# ╔═╡ 776343d0-2bd5-4cca-968b-5a7f92db23eb
md"""
We can also use the Euler-Maruyama method with fixed time step sizes.
"""

# ╔═╡ 91c01e44-c167-45c6-ade9-9fec73f98df8
let
	Random.seed!(42)
	parameters = (μ = 0.05, σ = 0.2)
	...
	ensemble = EnsembleProblem(sde;
		output_func = (sol, i) -> (h(sol.u[end], parameters, last(tspan)), false))
	sol = solve(ensemble, EM(), EnsembleThreads();
...
# ╔═╡ aaeefdc4-7c76-4e23-bac0-3eab132500fe
md"""
## Multilevel Monte Carlo implementation
"""

# ╔═╡ bb7a2004-811b-4a83-bc04-3f773e2badd5
md"""
First, we apply the MLMC implementation without ML - just EM at one level.
"""

# ╔═╡ 66eea53d-a16f-4357-9ac3-3b6b5f213c9a
md"""Next, we use a real MLMC approach with several levels."""

# ╔═╡ 2ae16b76-a954-4e67-b3b1-b1a316cfcc80
function mlmc(f, g, h, X0, parameters, tspan, N0, M0, refinements)
	
	return nothing
	# return (mean = "something that you compute",)
end

# ╔═╡ f2267682-943e-4ced-acda-d9a4a544c9c6
let
	Random.seed!(42)
	parameters = (μ = 0.05, σ = 0.2)
	...
	N0 = 100 # number of steps on coarse level
	M0 = 10^6 # number of samples on coarse level
	refinements = 0 # number of levels - 1

	results = mlmc(f, g, h, X0, parameters, tspan, N0, M0, refinements)
	estimate = results.mean

	md"""
	- Sample mean: $(round(estimate, digits = 3))
	- Exact value: $(round(exact_value(X0, parameters, tspan), digits = 3))
	"""
end

# ╔═╡ ce1788f8-c06c-4a85-91f8-6eaa4ab0aa47
let
	Random.seed!(42111)
	parameters = (μ = 0.05, σ = 0.2)
	...
	N0 = 10 # number of steps on coarse level
	M0 = 10^6 # number of samples on coarse level
	refinements = 8 # number of levels
...
manifest_format = "2.0"
project_hash = "fd5e77cbb588f06b553c2b6dd394d822ea3168b7"
